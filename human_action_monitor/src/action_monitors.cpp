...
ActionMonitors::ActionMonitors(ros::NodeHandle node_handle):node_handle_(node_handle) {
	...
	for (int i=0;i<actions_to_monitor_.size();i++) {
		ros::ServiceClient client_preconditions=node_handle_.serviceClient<shary3_msgs::CheckPreconditions>("/supervision/actions/"+actions_to_monitor_[i]+"/check_preconditions");
		ros::ServiceClient client_postconditions=node_handle_.serviceClient<shary3_msgs::SetPostconditions>("/supervision/actions/"+actions_to_monitor_[i]+"/set_postconditions");
	...
	for (int i=0; i<human_list_.size();i++) {
		ros::Publisher pub=node_handle_.advertise<shary3_msgs::Action>("/situation_assessment/"+human_list_[i]+"/action_performed",1000);
		...
		for (int i=0; i<object_list_.size();i++) {
			...
			for (int j=0;j<affordances.size();j++) {
				...
				for (int k=0;k<human_list_.size();k++) {
					...
					shary3_msgs::ParameterList parameter_list;
					shary3_msgs::Parameter target_parameter;
